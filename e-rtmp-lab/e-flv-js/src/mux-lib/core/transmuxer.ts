...
import TransmuxingEvent, { type DiscoveredTracks, type TransmuxingEventMap, type TransmuxingStatisticsInfo } from './transmuxing-events.js';
...
type WorkerMessage =
    ...
    | { msg: typeof TransmuxingEvent.INIT_SEGMENT; data: { type: TrackType; data: MSEInitSegment } }
    | { msg: typeof TransmuxingEvent.MEDIA_SEGMENT; data: { type: TrackType; data: MSEMediaSegment } }
    | { msg: typeof TransmuxingEvent.LOADING_COMPLETE }
    | { msg: typeof TransmuxingEvent.RECOVERED_EARLY_EOF }
    | { msg: typeof TransmuxingEvent.MEDIA_INFO; data: MediaInfo }
    | { msg: typeof TransmuxingEvent.METADATA_ARRIVED; data: unknown }
    | { msg: typeof TransmuxingEvent.SCRIPTDATA_ARRIVED; data: unknown }
    | { msg: typeof TransmuxingEvent.TIMED_ID3_METADATA_ARRIVED; data: unknown }
    | { msg: typeof TransmuxingEvent.STATISTICS_INFO; data: TransmuxingStatisticsInfo }
    | { msg: typeof TransmuxingEvent.TRACKS_DISCOVERED; data: DiscoveredTracks }
    | { msg: typeof TransmuxingEvent.IO_ERROR; data: { type: string; info: unknown } }
    | { msg: typeof TransmuxingEvent.DEMUX_ERROR; data: { type: string; info: string } }
    | { msg: typeof TransmuxingEvent.RECOMMEND_SEEKPOINT; data: number };
...
class Transmuxer {
    ...
    constructor(mediaDataSource: MediaDataSource, config: ResolvedPlayerConfig) {
        ...
        if (this._controller) {
            ...
            ctl.on(TransmuxingEvent.IO_ERROR, this._onIOError.bind(this));
            ctl.on(TransmuxingEvent.DEMUX_ERROR, this._onDemuxError.bind(this));
            ctl.on(TransmuxingEvent.INIT_SEGMENT, this._onInitSegment.bind(this));
            ctl.on(TransmuxingEvent.MEDIA_SEGMENT, this._onMediaSegment.bind(this));
            ctl.on(TransmuxingEvent.LOADING_COMPLETE, this._onLoadingComplete.bind(this));
            ctl.on(TransmuxingEvent.RECOVERED_EARLY_EOF, this._onRecoveredEarlyEof.bind(this));
            ctl.on(TransmuxingEvent.MEDIA_INFO, this._onMediaInfo.bind(this));
            ctl.on(TransmuxingEvent.METADATA_ARRIVED, this._onScriptMetadata.bind(this));
            ctl.on(TransmuxingEvent.SCRIPTDATA_ARRIVED, this._onScriptData.bind(this));
            ctl.on(TransmuxingEvent.TIMED_ID3_METADATA_ARRIVED, this._onTimedID3MetadataArrived.bind(this));
            ctl.on(TransmuxingEvent.STATISTICS_INFO, this._onStatisticsInfo.bind(this));
            ctl.on(TransmuxingEvent.TRACKS_DISCOVERED, this._onTracksDiscovered.bind(this));
            ctl.on(TransmuxingEvent.RECOMMEND_SEEKPOINT, this._onRecommendSeekpoint.bind(this));
        }
    }
    ...
    _onMediaInfo(mediaInfo: MediaInfo): void {
        Promise.resolve().then(() => {
            this._emitter.emit(TransmuxingEvent.MEDIA_INFO, mediaInfo);
        });
    }
    ...
    _onTimedID3MetadataArrived(data: unknown): void {
        Promise.resolve().then(() => {
            this._emitter.emit(TransmuxingEvent.TIMED_ID3_METADATA_ARRIVED, data);
        });
    }

    _onStatisticsInfo(statisticsInfo: TransmuxingStatisticsInfo): void {
        Promise.resolve().then(() => {
            this._emitter.emit(TransmuxingEvent.STATISTICS_INFO, statisticsInfo);
        });
    }
    ...
    _onRecommendSeekpoint(milliseconds: number): void {
        Promise.resolve().then(() => {
            this._emitter.emit(TransmuxingEvent.RECOMMEND_SEEKPOINT, milliseconds);
        });
    }
    ...
    _onWorkerMessage(event: MessageEvent<WorkerMessage>): void {
        ...
        switch (message.msg) {
            case TransmuxingEvent.INIT_SEGMENT:
            case TransmuxingEvent.MEDIA_SEGMENT:
                this._emitter.emit(message.msg, message.data.type, message.data.data);
                break;
            case TransmuxingEvent.LOADING_COMPLETE:
            case TransmuxingEvent.RECOVERED_EARLY_EOF:
                this._emitter.emit(message.msg);
                break;
            case TransmuxingEvent.MEDIA_INFO:
                Object.setPrototypeOf(message.data, MediaInfo.prototype);
                this._emitter.emit(message.msg, message.data);
                break;
            case TransmuxingEvent.METADATA_ARRIVED:
            case TransmuxingEvent.SCRIPTDATA_ARRIVED:
            case TransmuxingEvent.TIMED_ID3_METADATA_ARRIVED:
            case TransmuxingEvent.STATISTICS_INFO:
            case TransmuxingEvent.TRACKS_DISCOVERED:
                this._emitter.emit(message.msg, message.data);
                break;
            case TransmuxingEvent.IO_ERROR:
            case TransmuxingEvent.DEMUX_ERROR:
                this._emitter.emit(message.msg, message.data.type, message.data.info);
                break;
            case TransmuxingEvent.RECOMMEND_SEEKPOINT:
                this._emitter.emit(message.msg, message.data);
                break;
